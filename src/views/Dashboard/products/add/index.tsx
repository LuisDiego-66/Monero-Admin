...
const ProductWizard = ({ mode = 'create', productId }: ProductWizardProps) => {
  ...
  const [productCreated, setProductCreated] = useState<boolean>(!!productId)
  ...
  const handleStepClick = (stepIndex: number) => {
    // En modo creación, solo permite avanzar si ya se creó el producto
    if (isCreateMode) {
      if (stepIndex === 0) {
        setActiveStep(0)
  ...
  const onProductCreated = (newProductId: string) => {
    setProductCreated(true)
    setCreatedProductId(newProductId)

    // Auto-avanza a variantes en modo creación
    if (isCreateMode) {
      setActiveStep(1)
    }
  }
  ...
  return (
    <Card className='flex flex-col lg:flex-row'>
      ...
      <CardContent className='flex-1 pbs-6'>
        {getStepContent(activeStep, handleNext, handlePrev, mode, createdProductId, productCreated, onProductCreated)}
      </CardContent>
    </Card>
  )
}
